...
int Particles3Dcomm::communicate(VirtualTopology3D * ptVCT) {
  int rc = MPI_Barrier(ptVCT->getComm_P());
  // allocate buffers
  MPI_Status status;
  int new_buffer_size;
  int npExitingMax;
  ...
  while (np_current < nplast+1){
    ...
    std::unordered_set<int> local_faults = faults;
    int target_coords[3];
    target_coords[0] = x[np_current] / x_delta;
    target_coords[1] = y[np_current] / y_delta;
    target_coords[2] = z[np_current] / z_delta;
    if(x[np_current] < 0) target_coords[0] = 0;
    if(x[np_current] > Lx) target_coords[0] = x_size - 1;
    if(y[np_current] < 0) target_coords[1] = 0;
    ...
    else if (directions[target_coords[0]][target_coords[1]][target_coords[2]].parent[0] != -1)
    {
      ...
      bool closed_list[x_size][y_size][z_size];
      ...
      cell cells[x_size][y_size][z_size];
      for(int i = 0; i < x_size; i++)
        for(int j = 0; j < y_size; j++)
          for(int k = 0; k < z_size; k++) {
            cells[i][j][k].parent[0] = -1;
            cells[i][j][k].parent[1] = -1;
            cells[i][j][k].parent[2] = -1;
            cells[i][j][k].f = FLT_MAX;
            cells[i][j][k].g = FLT_MAX;
            cells[i][j][k].h = FLT_MAX;
          }
      cells[own_coords[0]][own_coords[1]][own_coords[2]].f = 0.0;
      cells[own_coords[0]][own_coords[1]][own_coords[2]].g = 0.0;
      cells[own_coords[0]][own_coords[1]][own_coords[2]].h = 0.0;
      cells[own_coords[0]][own_coords[1]][own_coords[2]].parent[0] = own_coords[0];
      cells[own_coords[0]][own_coords[1]][own_coords[2]].parent[1] = own_coords[1];
      cells[own_coords[0]][own_coords[1]][own_coords[2]].parent[2] = own_coords[2];
      std::set<std::pair<double, std::tuple<int, int, int>>> open_list;
      open_list.insert(make_pair(0.0, make_tuple(own_coords[0], own_coords[1], own_coords[2])));
      bool found_dest = false;
      while(!open_list.empty()) {
        std::pair<double, std::tuple<int, int, int>> current = *open_list.begin();
        open_list.erase(open_list.begin());
        int current_x = std::get<0>(current.second);
        int current_y = std::get<1>(current.second);
        int current_z = std::get<2>(current.second);
        closed_list[current_x][current_y][current_z] = true;
        double g_new, h_new, f_new;
        std::vector<std::tuple<int, int, int>> successors;
        if(current_x > 0 || ptVCT->getPeriods(0))
          successors.push_back(make_tuple((current_x > 0 ? current_x-1 : x_size-1), current_y, current_z));
        if(current_x < x_size - 1 || ptVCT->getPeriods(0))
          successors.push_back(make_tuple((current_x < x_size -1 ? current_x+1 : 0), current_y, current_z));
        if(current_y > 0 || ptVCT->getPeriods(1))
          successors.push_back(make_tuple(current_x, (current_y > 0 ? current_y-1 : y_size-1), current_z));
        if(current_y < y_size - 1 || ptVCT->getPeriods(1))
          successors.push_back(make_tuple(current_x, (current_y < y_size -1 ? current_y+1 : 0), current_z));
        if(current_z > 0 || ptVCT->getPeriods(2))
          successors.push_back(make_tuple(current_x, current_y, (current_z > 0 ? current_z-1 : z_size-1)));
        if(current_z < z_size - 1 || ptVCT->getPeriods(2))
          successors.push_back(make_tuple(current_x, current_y, (current_z < z_size -1 ? current_z+1 : 0)));

        for(auto successor : successors) {
          int successor_x = std::get<0>(successor);
          int successor_y = std::get<1>(successor);
          int successor_z = std::get<2>(successor);
          if(successor_x == target_coords[0] && successor_y == target_coords[1] && successor_z == target_coords[2]) {
            cells[successor_x][successor_y][successor_z].parent[0] = current_x;
            cells[successor_x][successor_y][successor_z].parent[1] = current_y;
            cells[successor_x][successor_y][successor_z].parent[2] = current_z;
            found_dest = true;
            break;
          }
          else if (!closed_list[successor_x][successor_y][successor_z]) {
            int rank, coords[3];
            coords[0] = successor_x;
            coords[1] = successor_y;
            coords[2] = successor_z;
            MPI_Cart_rank(comm, coords, &rank);
            if(local_faults.find(rank) == local_faults.end()) {
              g_new = cells[current_x][current_y][current_z].g + 1.0;
              h_new = abs(successor_x - target_coords[0]) + abs(successor_y - target_coords[1]) + abs(successor_z - target_coords[2]);
              f_new = g_new + h_new;
              if(cells[successor_x][successor_y][successor_z].f == FLT_MAX || cells[successor_x][successor_y][successor_z].f > f_new) {
                open_list.insert(make_pair(f_new, make_tuple(successor_x, successor_y, successor_z)));
                cells[successor_x][successor_y][successor_z].f = f_new;
                cells[successor_x][successor_y][successor_z].g = g_new;
                cells[successor_x][successor_y][successor_z].h = h_new;
                cells[successor_x][successor_y][successor_z].parent[0] = current_x;
                cells[successor_x][successor_y][successor_z].parent[1] = current_y;
                cells[successor_x][successor_y][successor_z].parent[2] = current_z;
              }
      ...
      if(!found_dest) {
        //We have a partition, we cannot overcome it :(
        printf("Partitioned :(\n");
        del_pack(np_current,&nplast);
      }
      else {
        int previous[3];
        current[0] = target_coords[0];
        current[1] = target_coords[1];
        current[2] = target_coords[2];
        memcpy(previous, cells[current[0]][current[1]][current[2]].parent, sizeof(previous));
        while(previous[0] != own_coords[0] || previous[1] != own_coords[1] || previous[2] != own_coords[2]) {
