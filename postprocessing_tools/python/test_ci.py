...
for ii in range(num_hdf_files):
    ...
    for file in glob.glob(dir_ref + "/proc" + str(ii) + ".hdf"):
        ...
        Bx_ref = np.array(Bx.get("cycle_9")); By_ref = np.array(By.get("cycle_9")); Bz_ref = np.array(Bz.get("cycle_9"))
        Ex_ref = np.array(Ex.get("cycle_9")); Ey_ref = np.array(Ey.get("cycle_9")); Ez_ref = np.array(Ez.get("cycle_9"))
    ...
    for file in glob.glob(dir_data + "/proc" + str(ii) + ".hdf"):
        ...
        Bx_data = np.array(Bx.get("cycle_9")); By_data = np.array(By.get("cycle_9")); Bz_data = np.array(Bz.get("cycle_9"))
        Ex_data = np.array(Ex.get("cycle_9")); Ey_data = np.array(Ey.get("cycle_9")); Ez_data = np.array(Ez.get("cycle_9"))
